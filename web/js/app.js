...
(function () {
    ...
    /**
     * Update the demand-action buttons and the badge to reflect the current mode.
     * mode: 'none' | 'route' | 'flow'
     * demand: the currently selected demand object (required when mode != 'none')
     */
    function setDemandButtonMode(mode, demand) {
        const btnRoute = document.getElementById('btn-highlight-demand');
        const btnFlow  = document.getElementById('btn-show-flow');
        const btnSegs  = document.getElementById('btn-show-segments');
        const badge    = document.getElementById('demand-selected-badge');

        if (mode === 'none') {
            [btnRoute, btnFlow, btnSegs].forEach(b => {
                b.disabled = true;
                b.classList.remove('btn-active', 'btn-flow-active');
            });
            badge.classList.add('hidden');
            return;
        }

        [btnRoute, btnFlow, btnSegs].forEach(b => { b.disabled = false; });

        // Active button styling
        btnRoute.classList.toggle('btn-active', mode === 'route');
        btnRoute.classList.remove('btn-flow-active');
        btnFlow.classList.toggle('btn-active', mode === 'flow');
        btnFlow.classList.toggle('btn-flow-active', mode === 'flow');

        // Badge text
        if (demand) {
            const modeLabel = mode === 'flow'
                ? '<span class="badge-mode mode-flow">Flow View</span>'
                : '<span class="badge-mode mode-route">Route View</span>';
            const wpStr = demand.waypoints.length > 0
                ? ` via [${demand.waypoints.map(w => 'v' + w).join(', ')}]`
                : '';
            badge.innerHTML = `D${demand.demand_id}: v${demand.source} → v${demand.target}${wpStr} &nbsp;|&nbsp; vol ${fmt(demand.volume, 2)} ${modeLabel}`;
    ...
    function buildSolverStateFromInstance(netData, tmData, scenarioData, solutionData) {
        ...
        function localDijkstra(source, target) {
            ...
            // Local ECMP flow computation used while building snapshots.
            // Splits the incoming quantity equally across all equal-cost outgoing
            // shortest-path arcs at each node, instead of duplicating full flow.
            function localComputeEcmpFlow(source, target, volume, excludeNodes, blockedArcs) {
                const dist = {};
                const visited = new Set();
                nodes.forEach(n => { dist[n.id] = Infinity; });
                dist[source] = 0;
                ...
                while (pq.length > 0) {
                    ...
                    for (const edge of graph[u]) {
                        if (blockedArcs && blockedArcs.has(edge.arcId)) continue;
                        ...
                        if (nd < dist[edge.to]) {
                            dist[edge.to] = nd;
                            pq.push({ node: edge.to, d: nd });
                        } else if (Math.abs(nd - dist[edge.to]) <= 1e-9) {
                            pq.push({ node: edge.to, d: nd });
                        }
                    }
                }
                ...
                const prevNodes = {};
                nodes.forEach(n => { prevNodes[n.id] = []; });

                arcs.forEach(a => {
                    if (blockedArcs && blockedArcs.has(a.id)) return;
                    if (dist[a.from] !== Infinity && dist[a.to] !== Infinity
                        && Math.abs(dist[a.from] + a.metric - dist[a.to]) < 1e-9) {
                        prevNodes[a.to].push(a.from);
                    }
                });

                const onDag = new Set([target]);
                const bfsQ = [target];
                while (bfsQ.length > 0) {
                    const v = bfsQ.shift();
                    for (const pred of prevNodes[v]) {
                        if (!onDag.has(pred)) {
                            onDag.add(pred);
                            bfsQ.push(pred);
                        }
                ...
                for (const u of topoOrder) {
                    ...
                    for (const edge of outEdges) {
                        arcFlow.set(edge.arcId, (arcFlow.get(edge.arcId) || 0) + flowPerEdge);
                        nodeFlow[edge.to] = (nodeFlow[edge.to] || 0) + flowPerEdge;
                    }
                }
        ...
        for (let t = 0; t < numTimeSlots; t++) {
            ...
            (tmData.demands || []).forEach((rawDemand, dIdx) => {
                ...
                if (volume > 0) {
                    ...
                    const visitedNodes = State.noRevisit ? new Set([source]) : null;
                    for (let i = 0; i < segEndpoints.length - 1; i++) {
                            const arcFlows = localComputeEcmpFlow(
                                segEndpoints[i],
                                segEndpoints[i + 1],
                                volume,
                                visitedNodes,
                            ...
                            arcFlows.forEach((flow, arcId) => {
                                flowAccum[arcId] += flow;
                            });

                            if (visitedNodes) {
                                arcFlows.forEach((_, arcId) => {
                    ...
                    demands.forEach(d => {
                        if (d.volume > 0) {
                            ...
                            for (let i = 0; i < segEndpoints.length - 1; i++) {
                                    ...
                                    const contrib = arcFlows.get(mostCongestedId) || 0;
                                    if (contrib > 1e-9) {
                                    d.is_candidate = true;
    ...
    function selectDemand(demandId) {
        ...
        setDemandButtonMode('route', demand);
    ...
    // SCENARIO TIMELINE
    // ========================================================================
    function getSnapshotForTime(t) {
        if (!State.data) return null;
        // Return the last iteration snapshot for a given time slot
        const snaps = State.data.snapshots.filter(s => s.time === t);
        return snaps.length ? snaps[snaps.length - 1] : null;
    }

    function budgetFillClass(used, allowed) {
        if (!allowed) return '';
        const pct = used / allowed;
    ...
    function updateTimeline() {
        ...
        const times = getAllTimeSlots();
        times.forEach(t => {
            const snap = getSnapshotForTime(t);
            const hasIntervention = snap && snap.intervention_arcs && snap.intervention_arcs.length > 0;
            const budgetUsed    = snap ? snap.budget.used    : 0;
            const budgetAllowed = snap ? snap.budget.allowed : 0;
            const fillPct = budgetAllowed > 0 ? Math.min(100, (budgetUsed / budgetAllowed) * 100) : 0;
            const fillCls = budgetFillClass(budgetUsed, budgetAllowed);

            const chip = document.createElement('div');
            chip.className = 'slot-chip' +
                (t === State.currentTimeSlot ? ' active' : '') +
                (hasIntervention ? ' has-intervention' : '');

            const ivCount = snap ? (snap.intervention_arcs || []).length : 0;
            const tooltipText = `t=${t} | Budget: ${budgetUsed}/${budgetAllowed}` +
                (hasIntervention ? ` | ${ivCount} arc(s) under maintenance` : '');

            chip.title = tooltipText;
            chip.innerHTML = `
                <span class="slot-chip-label">t=${t}</span>
                <div class="slot-chip-budget-bar">
                    <div class="slot-chip-budget-fill ${fillCls}" style="width:${fillPct}%"></div>
                </div>`;

            chip.addEventListener('click', function () {
                State.currentTimeSlot = t;
                State.currentSnapshotIdx = 0;
                updateAll();
            });
    ...
    function updateScenarioTab() {
        ...
        if (!State.data) {
            summaryEl.innerHTML = '';
            tableWrap.innerHTML = '<div class="empty-state">No data loaded</div>';
            return;
        }

        const times = getAllTimeSlots();
        // Deduplicate: one row per time slot (use last iteration snapshot)
        const rowData = times.map(t => {
            const snap = getSnapshotForTime(t);
            return {
                t,
                budgetUsed:    snap ? snap.budget.used    : 0,
                budgetAllowed: snap ? snap.budget.allowed : 0,
                interventionArcs: snap ? (snap.intervention_arcs || []) : [],
            };
        });

        // Summary KVs
        const totalSlots  = times.length;
        const ivSlots     = rowData.filter(r => r.interventionArcs.length > 0).length;
        const totalBudgetAllowed = rowData.reduce((s, r) => s + r.budgetAllowed, 0);
        const totalBudgetUsed    = rowData.reduce((s, r) => s + r.budgetUsed,    0);
        const totalIvArcs = rowData.reduce((s, r) => s + r.interventionArcs.length, 0);

        summaryEl.innerHTML = `
            <div class="scenario-kv">
                <span class="scenario-kv-label">Time Slots</span>
                <span class="scenario-kv-value">${totalSlots}</span>
            </div>
            <div class="scenario-kv">
                <span class="scenario-kv-label">Intervention Slots</span>
                <span class="scenario-kv-value" style="color:var(--accent-orange)">${ivSlots}</span>
            </div>
            <div class="scenario-kv">
                <span class="scenario-kv-label">Total Budget</span>
                <span class="scenario-kv-value">${totalBudgetUsed} / ${totalBudgetAllowed}</span>
            </div>
            <div class="scenario-kv">
                <span class="scenario-kv-label">Arc-Slots Blocked</span>
                <span class="scenario-kv-value" style="color:var(--accent-orange)">${totalIvArcs}</span>
            </div>`;
        ...
        rowData.forEach(row => {
            const isActive = row.t === State.currentTimeSlot;
            const hasIv    = row.interventionArcs.length > 0;
            const fillPct  = row.budgetAllowed > 0
                ? Math.min(100, (row.budgetUsed / row.budgetAllowed) * 100) : 0;
            const fillCls  = budgetFillClass(row.budgetUsed, row.budgetAllowed);
            const pctStr   = row.budgetAllowed > 0
                ? (row.budgetUsed / row.budgetAllowed * 100).toFixed(0) + '%' : '—';

            // Build arc tags
            let arcTagsHtml = '<span class="none-tag">none</span>';
            if (hasIv) {
                arcTagsHtml = '<div class="intervention-arcs">' +
                    row.interventionArcs.map(arcId => {
                        const arc = State.data.arcs.find(a => a.id === arcId);
                        const label = arc ? `a${arcId} (v${arc.from}→v${arc.to})` : `a${arcId}`;
                        return `<span class="intervention-arc-tag" title="Arc ${arcId}">${label}</span>`;
                    }).join('') +
                    '</div>';
            }

            html += `
                <tr class="scenario-row${isActive ? ' active-slot' : ''}${hasIv ? ' has-intervention' : ''}"
                    data-t="${row.t}">
                    <td><strong>t=${row.t}</strong></td>
                    <td>
                        <div class="budget-bar-wrap">
                            <span style="white-space:nowrap;font-size:10px">${row.budgetUsed} / ${row.budgetAllowed}</span>
                            <div class="budget-inline-bar">
                                <div class="budget-inline-fill ${fillCls}" style="width:${fillPct}%"></div>
                            </div>
                            <span style="width:28px;font-size:10px;color:var(--text-muted)">${pctStr}</span>
                        </div>
                    </td>
                    <td>${arcTagsHtml}</td>
                </tr>`;
        ...
        // Bind row click → navigate to time slot
        tableWrap.querySelectorAll('.scenario-row').forEach(row => {
    ...
    function bindEvents() {
        ...
        document.getElementById('chk-show-legend').addEventListener('change', function () {
            document.getElementById('graph-legend').classList.toggle('hidden', !this.checked);
        });
     ...
     */
    function computeEcmpFlow(source, target, volume, excludeNodes, blockedArcs) {
        const g = State.graph;
        if (!g) return new Map();
        ...
        while (pq.length > 0) {
            ...
            if (!g[u]) continue;
            for (const edge of g[u]) {
                // Skip intervention arcs (blocked by maintenance)
                if (blockedArcs && blockedArcs.has(edge.arcId)) continue;
                // Skip excluded nodes (no-revisit), but always allow the target
                if (excludeNodes && excludeNodes.has(edge.to) && edge.to !== target) continue;
    ...
    function computeDemandEcmpFlow(demand) {
        ...
        const snap = getCurrentSnapshot();
        const blockedArcs = new Set(snap ? (snap.intervention_arcs || []) : []);
        ...
        for (let i = 0; i < segmentEndpoints.length - 1; i++) {
            ...
            const segFlow = computeEcmpFlow(from, to, demand.volume, visitedNodes, blockedArcs);
    ...
    function showDemandFlowOverlay(demand) {
        ...
        const snap = getCurrentSnapshot();
        const overlayBlockedArcs = new Set(snap ? (snap.intervention_arcs || []) : []);
        const visitedNodesOverlay = State.noRevisit ? new Set([demand.source]) : null;
        for (let i = 0; i < segmentEndpoints.length - 1; i++) {
            const from = segmentEndpoints[i];
            const to = segmentEndpoints[i + 1];
            const color = segColors[i % segColors.length];
            const segFlow = computeEcmpFlow(from, to, demand.volume, visitedNodesOverlay, overlayBlockedArcs);
            ...
            // Count branching points and accumulate incoming flow per node
            const outCount = {};
            const nodeInFlow = {};
            nodeInFlow[from] = demand.volume;
            segFlow.forEach((fl, arcId) => {
                const arc = State.data.arcs.find(a => a.id === arcId);
                if (arc) {
                    outCount[arc.from] = (outCount[arc.from] || 0) + 1;
                    nodeInFlow[arc.to] = (nodeInFlow[arc.to] || 0) + fl;
                }
            });
            const branchEntries = Object.entries(outCount).filter(([, c]) => c > 1);
            const branchingNodes = branchEntries.length;
            ...
            if (branchEntries.length > 0) {
                flowHtml += '<div class="flow-ecmp-splits">';
                branchEntries.forEach(([nodeId, k]) => {
                    const inF = nodeInFlow[nodeId] !== undefined ? nodeInFlow[nodeId] : demand.volume;
                    const perBranch = inF / k;
                    flowHtml += `<div class="flow-split-row">
                        <span class="flow-split-node" style="color:${color}">v${nodeId}</span>
                        <span class="flow-split-arrow">→</span>
                        <span class="flow-split-info">${fmt(inF, 3)} ÷ ${k} = <strong>${fmt(perBranch, 3)}</strong> / path</span>
                    </div>`;
                });
                flowHtml += '</div>';
            }
        ...
        setDemandButtonMode('flow', demand);
    ...
    // ── Budget cost helpers ────────────────────────────────────────────────
    /**
     * Computes the budget cost for one demand between two SR paths.
     * Mirrors the C++ dist(SrPathBit_a, SrPathBit_b) exactly:
     *
     * The SrPathBit bitmask stores, for each SR path <s, w1, ..., wk, t>,
     * the set of consecutive directed node-pairs: {(s,w1),(w1,w2),...,(wk,t)}.
     * dist() = Hamming distance = |symmetric_difference(pairs_a, pairs_b)|.
     *
     * Special case: if the previous path is null (unrouted demand at t-1),
     * return segmentNum of the new path = [s, ...waypoints, t].length.
     *
     * Examples (source=s, target=t):
     *   direct <s,t>     → pairs = {(s,t)}
     *   waypoint <s,w,t> → pairs = {(s,w),(w,t)}
     *   dist(direct, waypoint-w) = |{(s,t),(s,w),(w,t)}| = 3
     *   dist(waypoint-A, waypoint-B) = 4  (when A≠B)
     *   dist(same, same) = 0
     */
    function distSrPaths(prevWaypoints, newWaypoints, source, target) {
        if (prevWaypoints === null || prevWaypoints === undefined) {
            // segmentNum = [s, ...waypoints, t].length
            return newWaypoints.length + 2;
        }

        // Build sets of directed consecutive-node-pair keys
        function pairSet(waypoints) {
            const nodes = [source, ...waypoints, target];
            const s = new Set();
            for (let i = 0; i < nodes.length - 1; i++) {
                s.add(nodes[i] + ',' + nodes[i + 1]);
            }
            return s;
        }

        const prev = pairSet(prevWaypoints);
        const curr = pairSet(newWaypoints);
        let cost = 0;
        prev.forEach(p => { if (!curr.has(p)) cost++; });
        curr.forEach(p => { if (!prev.has(p)) cost++; });
        return cost;
    }

    /**
     * Recomputes snap.budget.used for a given time slot snapshot, using the
     * previous time slot's last snapshot as reference.
     * Returns the new used value (0 for t == 0).
     */
    function recomputeBudgetUsed(snap) {
        if (!snap || snap.time === 0) return 0;
        const prevSnap = getSnapshotForTime(snap.time - 1);
        if (!prevSnap) return 0;

        let cost = 0;
        snap.demands.forEach(dCurr => {
            const dPrev = prevSnap.demands.find(d => d.demand_id === dCurr.demand_id);
            const prevWp = dPrev ? dPrev.waypoints : null;
            cost += distSrPaths(prevWp, dCurr.waypoints, dCurr.source, dCurr.target);
        });
        return cost;
    }
    ...
    function applyWaypointChanges() {
        ...
        // Compute old MLU and old budget cost
        const oldMlu = snap.objective.mlu;
        const oldBudgetUsed = snap.budget.used;
        const budgetAllowed = snap.budget.allowed;

        // Budget cost this single demand will add (relative to current waypoints)
        const thisChangeCost = snap.time > 0 ? distSrPaths(demand.waypoints, State.editingWaypoints, demand.source, demand.target) : 0;
        // Simulate new total budget used (replace old demand's contribution with new)
        const oldDemandCost = snap.time > 0 ? (() => {
            const prevSnap = getSnapshotForTime(snap.time - 1);
            const dPrev = prevSnap ? prevSnap.demands.find(d => d.demand_id === demand.demand_id) : null;
            return distSrPaths(dPrev ? dPrev.waypoints : null, demand.waypoints, demand.source, demand.target);
        })() : 0;
        const newBudgetUsed = oldBudgetUsed - oldDemandCost + thisChangeCost;
        const budgetExceeded = budgetAllowed > 0 && newBudgetUsed > budgetAllowed;
        ...
        const budgetColor = budgetExceeded ? 'var(--accent-red)' : (newBudgetUsed > 0 ? 'var(--accent-yellow)' : 'var(--accent-green)');
        const budgetNote = snap.time === 0 ? '<span style="color:var(--text-muted);font-size:10px">(t=0: no budget constraint)</span>' :
            budgetExceeded ? '<span style="color:var(--accent-red);font-size:10px">⚠ EXCEEDS BUDGET</span>' : '';

        const wpOld = demand.waypoints.length > 0 ? demand.waypoints.map(w => 'v' + w).join(', ') : '—';
        const wpNew = State.editingWaypoints.length > 0 ? State.editingWaypoints.map(w => 'v' + w).join(', ') : '—';
        ...
        resultEl.innerHTML = `
            ...
            <div class="wp-result-row" title="Waypoints changed from t-1 → t: dist(prev, new) = ${thisChangeCost}">
                <span>Waypoints:</span>
                <span style="font-size:10px">[${wpOld}] → [${wpNew}]</span>
            </div>
            <div class="wp-result-row" title="sum of dist(path_{t-1,d}, path_{t,d}) across all demands">
                <span>Budget cost (this demand):</span>
                <span style="color:${budgetColor};font-weight:700">${thisChangeCost}</span>
            </div>
            <div class="wp-result-row" title="Total budget used at this time slot after change">
                <span>Budget used / allowed:</span>
                <span style="color:${budgetColor};font-weight:700">${snap.time > 0 ? newBudgetUsed + ' / ' + budgetAllowed : 'N/A'}</span>
                ${budgetNote}
            </div>
        ...
        State._pendingNewBudgetUsed = newBudgetUsed;
    ...
    function confirmWaypointChanges() {
        ...
        // Recompute budget used for current time slot (now that waypoints are updated)
        snap.budget.used = recomputeBudgetUsed(snap);

        // Also recompute the next time slot budget, since it compares against this slot
        const nextSnap = getSnapshotForTime(snap.time + 1);
        if (nextSnap) {
            nextSnap.budget.used = recomputeBudgetUsed(nextSnap);
        }

        // Clean up
        State._pendingWaypointResult = null;
        State._pendingDemandId = undefined;
        State._pendingNewBudgetUsed = undefined;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const colorMap = {
            tokyonight: null,          // default :root, no attribute needed
        ...
        function applyTheme(colorKey) {
            const colorAttr = colorMap[colorKey];
            if (colorAttr) {
                root.setAttribute('data-color', colorAttr);
            } else {
        ...
        function applyMode(isDark) {
            if (isDark) {
                ...
                btnMode.textContent = '☀';
                btnMode.title = 'Switch to light mode';
            } else {
                root.setAttribute('data-theme', 'light');
                btnMode.textContent = '🌙';
                btnMode.title = 'Switch to dark mode';
            }
        }

        // Load saved preferences
        const savedColor = localStorage.getItem('tasr-color') || 'tokyonight';
        const savedDark  = localStorage.getItem('tasr-dark') !== 'false';  // default dark
        ...
        themeSelect.addEventListener('change', function () {
            ...
            localStorage.setItem('tasr-color', this.value);
        });

        btnMode.addEventListener('click', function () {
            const isDark = !root.hasAttribute('data-theme');
            applyMode(!isDark);
            localStorage.setItem('tasr-dark', String(!isDark));
        });
    });
    // ========================================================================
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ── Drag resize ────────────────────────────────────────────────────
        function makeDraggable(handle, getPanel, side) {
            let dragging = false;
            let startX = 0;
            let startW = 0;

            handle.addEventListener('mousedown', function (e) {
                if (e.target === toggleLeft || e.target === toggleRight) return;
            ...
            document.addEventListener('mousemove', function (e) {
                ...
                const minW  = side === 'left' ? 160 : 220;
                const maxW  = side === 'left' ? 520 : 640;
        ...
        makeDraggable(resizeLeft,  () => leftPanel,  'left');
        makeDraggable(resizeRight, () => rightPanel, 'right');
    });
})();
